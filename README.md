...
| 功能用例 | `/test-cases/functional` | 按模块、关键字、优先级、状态、项目归属、归属迭代和冒烟标记筛选及维护功能用例 | CRUD 与文件导入导出已持久化 |
...
| `listTestCases(query)` | 按类型、模块、关键字、优先级、状态、项目归属、归属迭代和冒烟标记查询用例 |
| `getTestCaseFilterOptions(type)` | 获取不受列表分页和当前筛选影响的项目归属与归属迭代选项 |
