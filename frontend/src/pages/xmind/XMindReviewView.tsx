...
export function XMindReviewView({ task, modules, onTaskChange }: XMindReviewViewProps) {
  ...
  const allCasesPassed = currentCases.length > 0 && passedCount === currentCases.length;
      ...
      <div className="xmind-review__header">
        ...
        <Space size="middle">
          ...
          <Button
            ...
            loading={merging}
            disabled={!allCasesPassed || !hasTargetModule}
            title={!allCasesPassed ? '全部用例审核通过后才能合并' : undefined}
            onClick={merge}
          >
            合并到用例库
          </Button>
