...
import { AuthClientError, createMemoryAuthClient } from '../../services/authClient';
...
it('账号已禁用时显示明确提示', async () => {
  const user = userEvent.setup();
  const client = {
    ...createMemoryAuthClient(),
    initialUser: null,
    login: vi.fn(async () => {
      throw new AuthClientError('Account is disabled', 403, 'account_disabled');
    }),
  };
  render(
  ...
  expect(await screen.findByText('当前用户已禁用')).toBeInTheDocument();
});
